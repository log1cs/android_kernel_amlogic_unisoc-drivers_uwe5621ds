...
{
	struct sprdwl_vif *vif = NULL, *found = NULL;
...
{
#ifdef CONFIG_ENC_WPA3
    #define NL80211_WPA_VERSION_3 (1 << 2)
#endif
#ifdef CONFIG_ENC_WAPI
...
void sprdwl_abort_running_scan(struct sprdwl_vif *vif)
{
        struct sprdwl_priv *priv = vif->priv;
        struct api_version_t *api = (&priv->sync_api)->api_array;
        ...
        if (!priv->scan_vif || priv->scan_vif != vif) {
                wl_err("%s vif not equal to scan_vif.\n", __func__);
                return;
        }
...
{
	struct scan_result *scan_node = NULL;
	struct sprdwl_priv *priv = vif->priv;
...
#endif
		netdev_info(vif->ndev,
			    "%s %s, passive disconnect, reason_code %d\n", __func__,
...
#endif
		netdev_info(vif->ndev,
			    "%s %s, active disconnect, reason_code %d\n", __func__,
			    vif->ssid, reason_code);
	} else {
		netdev_err(vif->ndev, "%s Unexpected event!\n", __func__);
		return;
			      ...
			      struct regulatory_request *request)
{
	...
	wl_info("[SC]%s %c%c initiator %d hint_type %d\n", __func__,
		request->alpha2[0], request->alpha2[1],
		request->initiator, request->user_reg_hint_type);
	...
	for (band = 0; band < IEEE80211_NUM_BANDS; band++) {
		...
		for (channel = 0; channel < sband->n_channels; channel++) {
			...
			reg_rule =
			     sprd_freq_reg_info_regd(MHZ_TO_KHZ(chan->center_freq), pRegdom, MHZ_TO_KHZ(20));
#else
			reg_rule =
